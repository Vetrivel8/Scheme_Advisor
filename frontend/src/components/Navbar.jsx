...
export default function Navbar() {
  ...
  const resetSession = () => {
    ...
    // Also clear specific session markers
    sessionStorage.removeItem("activeWizardSession");
    console.log("Navbar: Session reset.");
  };
    ...
    <nav className="sticky top-0 z-50 bg-white border-b border-gray-100 text-gray-800 shadow-sm">
      <div className="max-w-7xl mx-auto flex items-center justify-between px-6 h-16">
        ...
        <div className="flex items-center gap-8 text-sm font-semibold">
          ...
          <Link to="/schemes" className="hover:text-primary transition-colors">{t.browseAll}</Link>

          {!token ? (
            <div className="flex items-center gap-6 ml-4">
              <Link to="/login" className="text-gray-600 hover:text-primary uppercase tracking-widest text-[11px]">{t.login}</Link>
            ...
            <div className="flex items-center gap-4 ml-4">
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl border border-gray-200 hover:border-primary/5 hover:bg-primary/5 transition-all text-gray-500 hover:text-primary"
              >
                <Languages size={15} />
                ...
                {notifOpen && (
                  <div className="absolute right-0 mt-3 w-80 bg-white border border-gray-100 rounded-2xl shadow-xl py-0 z-50 animate-in fade-in slide-in-from-top-2 duration-200 overflow-hidden">
                    <div className="px-4 py-4 border-b border-gray-50 flex items-center justify-between bg-gray-50/50">
                      <h3 className="text-xs font-black text-gray-900 uppercase tracking-widest">{t.notifications}</h3>
                      {unreadCount > 0 && (
                        <button onClick={markAllRead} className="text-[10px] font-bold text-primary hover:underline">{t.markAllRead}</button>
                      )}
                    </div>
                    ...
                    <div className="max-h-96 overflow-y-auto">
                      {notifications.length === 0 ? (
                        <div className="p-10 text-center">
                          ...
                          <p className="text-xs text-gray-400 font-medium">{t.noNotifications}</p>
                        </div>
                      ) : (
                        notifications.map(n => (
              ...
              <div className="relative" ref={dropdownRef}>
                ...
                >
                  <div className="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center text-primary border border-primary/20">
                    <span className="font-black text-xs">{user.name?.[0]?.toUpperCase() || 'U'}</span>
                  </div>
                ...
                {dropdownOpen && (
                  <div className="absolute right-0 mt-3 w-56 bg-white border border-gray-100 rounded-2xl shadow-xl py-2 z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                    <div className="px-4 py-3 border-b border-gray-50 mb-1">
                      ...
                      <p className="text-[10px] text-gray-400 font-medium mt-1 truncate">{user.email || "No email"}</p>
                    </div>
                    ...
                    >
                      <LayoutDashboard size={14} /> {t.myOverview}
                    </Link>
                    <Link 
                    ...
                    <Link 
                      ...
                      className="flex items-center gap-3 px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors"
                    >
                      <Bookmark size={14} /> {t.savedSchemes}
                    </Link>
                    <Link 
                      ...
                      className="flex items-center gap-3 px-4 py-2.5 text-[11px] font-black uppercase tracking-widest text-gray-600 hover:bg-primary/5 hover:text-primary transition-colors"
                    >
                      <Briefcase size={14} /> {t.trackApplication}
                    </Link>
